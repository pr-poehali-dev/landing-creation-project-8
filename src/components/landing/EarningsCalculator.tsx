      ...
      <div className="container mx-auto px-4 md:px-6 max-w-6xl">
        ...
        <Card className="mx-auto border-2 border-primary/30 shadow-2xl bg-background">
          <CardHeader className="text-center pb-2 pt-3">
            <h3 className="text-base md:text-lg font-bold mb-2">Выбери тип курьера</h3>
            <div className="grid grid-cols-3 gap-3 md:gap-5">
              {(Object.keys(courierData) as CourierType[]).map((type) => (
                <button
                  key={type}
                  onClick={() => setCourierType(type)}
                  className={`flex flex-col items-center gap-1 p-1.5 rounded-lg transition-all border-2 overflow-hidden ${
                    courierType === type
                ...
                >
                  ...
                  <span className="text-[11px] font-bold">{courierData[type].title}</span>
                </button>
              ))}
            </div>
          </CardHeader>
          ...
          <CardContent className="space-y-2 pb-3">
            <div className="bg-muted/20 rounded-lg p-2">
              <div className="flex flex-col md:flex-row md:justify-between md:items-center gap-1.5 md:gap-0 mb-1.5">
                <label className="text-xs md:text-sm font-bold">Сколько часов готов работать?</label>
                <div className="flex items-center gap-1.5 rounded px-2 py-1 bg-primary self-start">
                  ...
                  <span className="text-xs text-secondary font-bold">ч/день</span>
                </div>
              </div>
              <Slider
                value={hoursPerDay}
            ...
            <div className="grid md:grid-cols-2 gap-2">
              <div className="bg-primary rounded-lg p-2 text-secondary">
                <div className="text-center">
                  <p className="text-[10px] font-semibold mb-0.5">Доход в день</p>
                  <p className="text-xl md:text-2xl font-bold mb-1">
                    {earningsPerDay.toLocaleString('ru-RU')} ₽
                  </p>
                  <div className="space-y-0 text-[10px] text-secondary/90">
              ...
              <div className="bg-primary rounded-lg p-2 text-secondary">
                <div className="text-center">
                  <p className="text-[10px] font-semibold mb-0.5">Доход в месяц</p>
                  <p className="text-2xl md:text-3xl font-bold mb-0.5">
                    {earningsPerMonth.toLocaleString('ru-RU')} ₽
                  </p>
                  <p className="text-[10px] text-secondary/90">при 22 рабочих днях</p>
                </div>
              </div>
            </div>

            <div className="text-center">
              <Button size="default" className="bg-primary hover:bg-primary/90 text-secondary font-bold w-full md:w-auto animate-pulse-scale text-sm" asChild>
