      ...
      <div className="container mx-auto px-4 md:px-6 max-w-6xl">
        ...
        <Card className="mx-auto border-2 border-primary/30 shadow-2xl bg-background">
          <CardHeader className="text-center pb-3">
            <h3 className="text-base md:text-lg font-bold mb-3">Выбери тип курьера</h3>
            <div className="grid grid-cols-3 gap-4 md:gap-6">
              {(Object.keys(courierData) as CourierType[]).map((type) => (
                <button
                  key={type}
                  onClick={() => setCourierType(type)}
                  className={`flex flex-col items-center gap-1.5 p-2 rounded-xl transition-all border-2 overflow-hidden ${
                    courierType === type
                ...
                >
                  ...
                  <span className="text-xs font-bold">{courierData[type].title}</span>
                </button>
              ))}
            </div>
          </CardHeader>
          ...
          <CardContent className="space-y-3">
            <div className="bg-muted/20 rounded-xl p-3">
              <div className="flex flex-col md:flex-row md:justify-between md:items-center gap-2 md:gap-0 mb-2">
                <label className="text-sm font-bold">Сколько часов готов работать?</label>
                <div className="flex items-center gap-2 rounded-lg px-3 py-1.5 bg-primary self-start">
                  ...
                  <span className="text-sm text-secondary font-bold">ч/день</span>
                </div>
              </div>
              <Slider
                value={hoursPerDay}
            ...
            <div className="grid md:grid-cols-2 gap-3">
              <div className="bg-primary rounded-xl p-3 text-secondary">
                <div className="text-center">
                  <p className="text-xs font-semibold mb-1">Доход в день</p>
                  <p className="text-2xl md:text-3xl font-bold mb-2">
                    {earningsPerDay.toLocaleString('ru-RU')} ₽
                  </p>
                  <div className="space-y-0.5 text-[11px] text-secondary/90">
              ...
              <div className="bg-primary rounded-xl p-3 text-secondary">
                <div className="text-center">
                  <p className="text-xs font-semibold mb-1">Доход в месяц</p>
                  <p className="text-3xl md:text-4xl font-bold mb-1">
                    {earningsPerMonth.toLocaleString('ru-RU')} ₽
                  </p>
                  <p className="text-[11px] text-secondary/90">при 22 рабочих днях</p>
                </div>
              </div>
            </div>
